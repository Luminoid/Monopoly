    ...
    public static void userInfoInput() {
        ...
        while (true) {
            try {
                ...
                if (num >= 2 && num <= 4) {
                    Kernel.createInstance(num, false);
                    break;
                } else {
        ...
        // Cash
        System.out.println("请输入玩家的初始现金(默认为20000)：");
        double cash;
        while (true) {
            try {
                System.out.print(">> ");
        ...
        // Player name
        for (int i = 1; i <= Kernel.getInstance().getPlayerNum(); i++) {
            System.out.println("请输入玩家" + i + "的名字：");
            System.out.print(">> ");
            String name = scanner.nextLine().trim();
            Kernel.getInstance().addPlayer(new Player(name, cash));
        }
        // Start time
        ...
        while (true) {
            try {
                ...
                String s = scanner.nextLine();
                if (s.length() == 0) {
                    Kernel.getInstance().setDate(2016, 4, 1);
                } else {
                    String[] dateArr = s.split(" ");
                    int year = Integer.parseInt(dateArr[0]);
                    int month = Integer.parseInt(dateArr[1]);
                    int day = Integer.parseInt(dateArr[2]);
                    Kernel.getInstance().setDate(year, month, day);
                }
                break;
            } catch (Exception ex) {
                System.out.println("您的输入格式不正确，请重新输入：");
        ...
        while (true) {
            try {
                ...
                String s = scanner.nextLine();
                if (s.length() == 0) {
                    Kernel.getInstance().setEndDate(2016, 8, 1);
                } else {
                    String[] dateArr = s.split(" ");
                    int year = Integer.parseInt(dateArr[0]);
                    int month = Integer.parseInt(dateArr[1]);
                    int day = Integer.parseInt(dateArr[2]);
                    Kernel.getInstance().setEndDate(year, month, day);
                }
                break;
            } catch (Exception ex) {
                System.out.println("您的输入格式不正确，请重新输入：");
